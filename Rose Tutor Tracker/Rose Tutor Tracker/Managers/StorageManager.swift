...
class StorageManager {
    ...
    func uploadProfilePhoto(uid: String, image: UIImage, changeListener: @escaping (() -> Void)) {
        guard let imageData = ImageUtils.resize(image: image) else {
            print("Converting the image to data failed!")
            return
        }
        let photoRef = _storageRef.child("images").child(uid)
        photoRef.putData(imageData, metadata: nil) { metadata, error in
            ...
            photoRef.downloadURL { downloadUrl, error in
                ...
                UserDocumentManager.shared.currStudent!.storageUriString = downloadUrl?.absoluteString ?? ""
